# -*- coding: utf-8 -*-
"""
Created on Thu Dec 18 23:19:35 2025
...
# =========================
# 3D Sample generator (FCC single crystal volume)
# =========================
def generate_fcc_single_crystal_volume(D: int, H: int, W: int, a_px: int = 24, sigma_px: float = 1.1,
                                       base_level: float = 80.0, atom_intensity: float = 9000.0) -> np.ndarray:
    '''
    Generate a 3D FCC single-crystal 'potential/density' volume V[z,y,x] as float32.
    Crystal is aligned with the volume axes. At (a,b)=(0,0) this is near a [001] zone orientation.
    '''
    ...
    # Gaussian blur via FFT (3D) for fast atom-shape rendering
...
class STEMServer(object):
    def __init__(self):
        ...
        D, H, W = 64, 768, 768
        print(f"[DT] Generating 3D FCC volume {D}x{H}x{W} (one-time) ...")
        self.vol = generate_fcc_single_crystal_volume(D, H, W, a_px=24, sigma_px=1.1, base_level=80.0, atom_intensity=9000.0)
