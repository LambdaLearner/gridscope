...
| Magnification | `set_magnification` | 1 – 20 000 kx | 100 kx | atomic columns resolve ≳4000 kx @512 px |
| Resolution | `set_resolution` | {512, 1024, 2048} | 512 | higher → columns at lower mag, slower |
...
- **Damage/contamination speed depends on FOV and resolution**, not just current/dwell — a
  ...
  instrument. (Measured: at fixed FOV, going 512→1024 px took ~5-frame darkening from ~27 %→~72 %.)
...
physical distances at 512 vs 1024 vs 2048.
...
- **Beam shift** (`set_beam({"x":…, "y":…})`) and **any "move by N pixels" / "click-to-position"**
  ...
  The same on-screen drag is **4× smaller physically at 2048 px than at 512 px** (for the same
